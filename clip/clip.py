...
def export_image_onnx(model_name: str, save_path: str = None, batch_size: int = None, opset_version: int = 12):
    ...
    if not save_path:
        batch_str = "dynamic" if not batch_size else str(batch_size)
        save_path = os.path.expanduser(f"~/.cache/clip/{model_name}Image_batch_{batch_str}.onnx")
    if os.path.exists(save_path) and os.path.isfile(save_path):
        print(f"model onnx already exsits in {save_path}, skip export")
    else:
        batch_size = batch_size if batch_size else 1
...
def export_text_onnx(model_name: str, save_path: str = None, batch_size: int = None, opset_version: int = 12):
    ...
    if not save_path:
        batch_str = "dynamic" if not batch_size else str(batch_size)
        save_path = os.path.expanduser(f"~/.cache/clip/{model_name}Text_batch_{batch_str}.onnx")
    if os.path.exists(save_path) and os.path.isfile(save_path):
        print(f"model onnx already exsits in {save_path}, skip export")
    else:
        batch_size = batch_size if batch_size else 1
